    ...
    <view v-for="b in pageBlocks" :key="b.id">
      ...
      >
        <swiper-item v-for="bn in banners" :key="bn.id">
          <image class="banner-image" :src="staticUrl(bn.image_url)" mode="aspectFill"></image>
        </swiper-item>
      </swiper>

      <!-- 矩阵账号 -->
      <view v-else-if="b.type === 'builtin_matrix_accounts'" class="panel">
        <view class="panel-head">
          <view class="head-bar"></view>
          <text class="panel-title">{{ b.props.title || '矩阵账号' }}</text>
      ...
      <!-- 今日数据分析 -->
      <view v-else-if="b.type === 'builtin_today_stats'" class="panel">
        <view class="panel-head">
          <view class="head-bar"></view>
          <text class="panel-title">{{ b.props.title || '今日数据分析' }}</text>
      ...
      <!-- 分平台视频 + DeepSeek 分析 -->
      <view v-else-if="b.type === 'builtin_video_feed'" class="panel">
...
<script>
import { getAccounts, getBanners, getDecorBlocks, getTodayStats, getVideos, staticUrl } from '@/utils/api.js'
...
export default {
  ...
  methods: {
    async loadAll() {
      const [pageBlocks, banners, accounts, today, videos] = await Promise.all([
        getDecorBlocks('frontend', 'home').catch(() => []),
        getBanners().catch(() => []),
        getAccounts().catch(() => []),
